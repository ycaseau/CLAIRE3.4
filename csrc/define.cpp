/***** CLAIRE Compilation of file c:\claire\v3.4\src\meta\define.cl 
...
//
/* The c++ function for: self_print(self:Definition) [NEW_ALLOC] */
...
// -------------- method definition ----------------------------------
//
/* The c++ function for: self_print(self:Defmethod) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
// -------------- array definition -----------------------------------
/* The c++ function for: self_print(self:Defarray) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
// -------------- rule definition ------------------------------------
/* The c++ function for: self_print(self:Defrule) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
//
/* The c++ function for: complete(self:object,%l:list) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
//
/* The c++ function for: self_eval(self:Defobj) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
/* The c++ function for: attach_comment(x:any) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
//
/* The c++ function for: iClaire/extract_pattern(x:any,path:list) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
//
/* The c++ function for: extract_item(x:any,y:any) [NEW_ALLOC] */
...
// patch.
/* The c++ function for: extract_pattern_nth(l:list,path:list) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
                ...
                { OID  y = (*(OBJECT(bag,l2)))[n];
                  { OID  g0080UU;
                    { if (INHERIT(OWNER(y),Language._Set))
                       { OID  v = GC_OID(extract_pattern_any(GC_OID((*(OBJECT(Construct,y)->args))[1]),GC_OBJECT(list,((list *) copy_bag(path)))->addFast(GC_OID((*Kernel.nth)(l1,
                          n)))));
                  ...
                  { OID  y = (*(l))[n];
                    ...
                    if (INHERIT(OWNER(y),Language._Call))
                     { if (((OBJECT(Call,y)->selector == Kernel._equal) ? ((OBJECT(Call,y)->args->length == 2) ? CTRUE: CFALSE): CFALSE) != CTRUE)
                      ...
                      p= _oid_(make_a_property_any(GC_OID((*(OBJECT(Call,y)->args))[1])));
          ...
          { lambda * lb = GC_OBJECT(lambda,lambda_I_list(lv2,GC_OID((*(OBJECT(bag,(*Core.args)(x))))[2])));
            ...
            { ClaireHandler c_handle = ClaireHandler();
              ...
              { c_handle.catchIt();{ princ_string("The type expression ");
                  print_any(x);
                  princ_string(" is not valid ... \n");
                  princ_string("context: lambda = ");
                  print_any(_oid_(lb));
                  princ_string(", lvars = ");
                  { OID  g0093UU;
...
//
/* The c++ function for: iClaire/type!(x:any) [NEW_ALLOC] */
  ...
  { OID Result = 0;
    if ((*(self->args))[1] != _oid_(ClEnv))
     Result = (*Language.eval_rule)(_oid_(self));
        ...
        OID  ru = GC_OID(get_symbol(self->ident));
              ...
              { { Call * _CL_obj = ((Call *) GC_OBJECT(Call,new_object_class(Language._Call)));
                  ...
                  { Call * g0112 = _CL_obj; 
                    ...
                    { OID v_bag;
                      ...
                      { { List * _CL_obj = ((List *) GC_OBJECT(List,new_object_class(Language._List)));
                          (_CL_obj->args = list::alloc(3,GC_OID(_string_(string_I_symbol(n))),
                            x,
                            y));
                          add_I_property(Kernel.instances,Language._List,11,_oid_(_CL_obj));
                          v_bag = _oid_(_CL_obj);
...
/* The c++ function for: eval_if_write(R:relation) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
{ GC_BIND;
  { OID  l = GC_OID(nth_table1(Language.demons,_oid_(R)));
    ...
    { ITERATE(v);
      for (START(lvar); NEXT(v);)
      put_property2(Kernel.range,OBJECT(ClaireObject,v),_oid_(class_I_type(OBJECT(ClaireType,(*Kernel.range)(v)))));
      } 
    if (((R->inverse == (NULL)) ? CTRUE : CFALSE) != CTRUE)
     { if (multi_ask_any(_oid_(R)) != CTRUE)
       { { OID  g0124UU;
        ...
        GC_OBJECT(list,l1);} 
      l1= GC_OBJECT(list,l1->addFast(GC_OID(_oid_(putCall_relation2(R->inverse,(*(lvar))[2],(*(lvar))[1])))));
      } 
      ...
      { lambda * V_CL0127;{ OID  g0128UU;
          if (eventMethod_ask_relation2(R) == CTRUE)
           ...
           { If * _CL_obj = ((If *) GC_OBJECT(If,new_object_class(Language._If)));
            ...
            V_CL0127 = lambda_I_list(list::alloc(2,(*(lvar))[1],(*(lvar))[2]),g0128UU);
          } 
...
/* The c++ function for: eventMethod(p:property) [NEW_ALLOC+SLOT_UPDATE] */
...
{ GC_BIND;
  { method * m = add_method_property(p,
    ...
    store_object(m,
      ...
      p->if_write,
      CFALSE);
    (m->functional = make_function_string(append_string(GC_STRING(string_I_symbol(p->name)),"_write")));
    } 
  GC_UNBIND;} 
...
/* The c++ function for: interface(p:property) [NEW_ALLOC+SLOT_UPDATE] */
...
/* The c++ function for: interface(c:class,l:listargs) [NEW_ALLOC] */
