/***** CLAIRE Compilation of file ccmain.cl 
         [version 3.4.0 / safety 5] Sat Aug 24 00:20:46 2013 *****/
    ...
    { ClaireHandler c_handle = ClaireHandler();
      ...
      { { (Reader._starfs_star->value= _string_("\\"));
          ...
          update_property(Optimize.libraries_dir,
            ...
            17,
            Kernel._object,
            _oid_(list::alloc(Kernel._any,3,_string_("c:\\claire\\v3.4\\bin\\public\\ntv"),
              _string_("c:\\claire\\v3.4\\bin\\debug\\ntv"),
              _string_("c:\\claire\\v3.4\\bin\\public\\ntv"))));
          (Optimize.compiler->headers_dir = "c:\\claire\\v3.4\\bin\\include");
          ...
          if (equal_string(_Zcm,"") != CTRUE)
           { module * m = string2module_string(_Zcm);
            ...
            (Optimize.claire_modules->value= _oid_(GC_OBJECT(list,OBJECT(list,Optimize.claire_modules->value))->addFast(_oid_(m))));
            (*Reader.load)(GC_OID(value_string("Compile")));
            if (equal_string(_Zout,"") != CTRUE)
             (m->external = _Zout);
            load_module(m);
            if (dblevel < 1)
           ...
           (*Core.call)(GC_OID(value_string("jcmakefile")),
