/***** CLAIRE Compilation of file c:\claire\v3.4\src\meta\call.cl 
         [version 3.4.0 / safety 5] Sun Sep 29 07:16:33 2013 *****/
...
{ GC_BIND;
  { OID Result = 0;
    { property * p = self->selector;
      ...
      else { OID  z = GC_OID(slot_get_object(OBJECT(ClaireObject,x),CLREAD(slot,s,index),CLREAD(slot,s,srange)));
          if ((z != CNULL) || 
...
//
/* The c++ function for: printe(self:any,s:property) [NEW_ALLOC+SLOT_UPDATE] */
...
/* The c++ function for: self_print(self:Assign) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
/* The c++ function for: self_eval(self:Assign) [NEW_ALLOC] */
...
// global variables
//
/* The c++ function for: self_print(self:Gassign) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
/* The c++ function for: self_eval(self:Gassign) [NEW_ALLOC] */
...
//
/* The c++ function for: self_print(self:And) [NEW_ALLOC] */
...
// or expression
//
/* The c++ function for: self_print(self:Or) [NEW_ALLOC] */
...
//
/* The c++ function for: self_print(self:Call_method) [NEW_ALLOC] */
...
/* The c++ function for: self_eval(self:Call_slot) [NEW_ALLOC] */
...
/* The c++ function for: self_eval(self:Call_array) [NEW_ALLOC] */
...
/* The c++ function for: self_eval(self:Call_table) [NEW_ALLOC] */
...
/* The c++ function for: self_eval(self:Update) [NEW_ALLOC] */
...
//
/* The c++ function for: self_print(self:Super) [NEW_ALLOC+SLOT_UPDATE] */
...
/* The c++ function for: self_eval(self:Super) [NEW_ALLOC] */
...
/* The c++ function for: self_eval(self:Cast) [NEW_ALLOC] */
...
// of structures
//
/* The c++ function for: self_print(self:Return) [NEW_ALLOC] */
...
/* The c++ function for: substitution(self:any,x:Variable,val:any) [BAG_UPDATE+SLOT_UPDATE] */
...
/* The c++ function for: occurrence(self:any,x:Variable) [0] */
...
//
/* The c++ function for: instruction_copy(self:any) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
OID  instruction_copy_any(OID self)
{ GC_BIND;
  { OID Result = 0;
    if (INHERIT(OWNER(self),Kernel._bag))
     { bag * l = GC_OBJECT(bag,copy_bag(OBJECT(bag,self)));
      { int  i = 1;
     ...
     { Instruction * o = GC_OBJECT(Instruction,((Instruction *) copy_object(OBJECT(ClaireObject,self))));
      { OID gc_local;
        ...
        for (START(s_support); NEXT(s);)
        { GC_LOOP;
          put_slot(OBJECT(slot,s),o,instruction_copy_any(GC_OID(get_slot(OBJECT(slot,s),OBJECT(ClaireObject,self)))));
          GC_UNLOOP;} 
        } 
      Result = _oid_(o);
      } 
