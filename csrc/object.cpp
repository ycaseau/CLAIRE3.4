/***** CLAIRE Compilation of file c:\claire\v3.4\src\meta\object.cl 
...
{ princ_string("CLAIRE v3.");
  ...
  princ_string(" Copyright (C) 1994-2013 Yves Caseau. All Rights Reserved.\n");
  princ_string("use and redistribution in source code or binary forms are permitted\n");
  princ_string("resale is not permitted without the explicit agreement of Yves Caseau\n");
...
/* The c++ function for: push_debug(prop:property,arity:integer,start:integer) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
/* The c++ function for: put(self:property,x:object,y:any) [NEW_ALLOC+SLOT_UPDATE] */
...
/* The c++ function for: get(a:table,x:any) [0] */
...
/* The c++ function for: nth=(a:table,x:any,y:any) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
/* The c++ function for: nth_put(a:table,x:any,y:any) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
{ if ((((a->if_write == CNULL) ? CTRUE : CFALSE) != CTRUE) && 
  ...
  { fastcall_relation2(a,x,y);
    ...
    if (multi_ask_any(_oid_(a)) == CTRUE)
     { OID  r = GC_OID(get_property(Kernel.inverse,a));
      ...
      { OID  g0052UU;
        if (OBJECT(set,y)->length == 0)
         ...
         g0052UU = _oid_(make_list_integer2(0,GC_OBJECT(ClaireType,of_extract_type(a->range)),0));
...
/* The c++ function for: put(a:table,x:any,y:any) [0] */
...
/* The c++ function for: add(a:table,x:any,y:any) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
/* The c++ function for: delete(a:table,x:any,y:any) [BAG_UPDATE+SLOT_UPDATE] */
...
/* The c++ function for: nth=(a:table,x:any,y:any,z:any) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
/* The c++ function for: erase(a:table) [NEW_ALLOC+BAG_UPDATE] */
...
/* The c++ function for: make_table(%domain:type,%range:type,%default:any) [NEW_ALLOC] */
...
/* The c++ function for: domain!(x:restriction) [0] */
...
/* The c++ function for: reify(l:listargs) [0] */
...
/* The c++ function for: self_print(self:general_error) [NEW_ALLOC] */
...
/* The c++ function for: self_print(self:system_error) [NEW_ALLOC] */
...
{ GC_RESERVE(6);  // v3.0.55 optim !
  { char * s = self;
    ...
    list * l = GC_OBJECT(list,((list *) copy_bag(larg)));
    { OID gc_local;
      ...
      { GC_LOOP;
        { ClaireChar * m = _char_(s[(n+1) - 1]);
          ...
          if ('A' == ((char) m->ascii))
           (*Kernel.princ)(GC_OID(car_list(l)));
          else if ('S' == ((char) m->ascii))
           print_any(GC_OID(car_list(l)));
          ...
          if (((char) m->ascii) != '%')
           GC__ANY(l = skip_list(l,1), 5);
          GC__STRING(s = substring_string(s,(n+2),1000), 3);
