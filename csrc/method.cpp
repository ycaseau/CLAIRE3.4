/***** CLAIRE Compilation of file c:\claire\v3.4\src\meta\method.cl 
      ...
      if (((self->formula == (NULL)) ? CTRUE : CFALSE) != CTRUE)
       { int  retour = ClEnv->base;
        ...
        Result = val;
          ...
          if (st_ask == CTRUE)
           push_debug_property(self->selector,n,start);
          val= GC_OID(stack_apply_function(self->evaluate,self->srange,start,i));
          if (st_ask == CTRUE)
           pop_debug_property(self->selector,0,val);
          if ((ClEnv->debug_I != -1) && 
              (belong_to(val,_oid_(self->range)) != CTRUE))
...
/* The c++ function for: inlineok?(self:method,s:string) [NEW_ALLOC+SAFE_RESULT] */
...
/* The c++ function for: put(s:slot,x:object,y:any) [SLOT_UPDATE] */
...
/* The c++ function for: get(self:property,x:object) [0] */
...
/* The c++ function for: read(self:property,x:object) [0] */
...
// verifying
/* The c++ function for: hold?(self:property,x:object,y:any) [0] */
...
// to remove
/* The c++ function for: put(p:property,x:object,n:integer,s:class,y:any) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
{ GC_BIND;
  { OID  old = GC_OID(slot_get_object(x,n,s));
...
/* The c++ function for: update+(self:relation,x:any,y:any) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
    ...
    if ((r != NULL) && 
     ...
     { if (INHERIT(r->isa,Kernel._property))
       { ClaireObject * s = GC_OBJECT(ClaireObject,_at_property1(((property *) r),OWNER(y)));
        if (Kernel._slot == s->isa)
         { OID  old_y = GC_OID(get_slot(((slot *) s),OBJECT(ClaireObject,y)));
...
/* The c++ function for: update-(r:relation,x:any,y:any) [BAG_UPDATE+SLOT_UPDATE] */
...
/* The c++ function for: add!(self:property,x:object,n:integer,y:any) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
void  add_I_property(property *self,ClaireObject *x,int n,OID y)
{ if (((self->if_write == CNULL) ? CTRUE : CFALSE) != CTRUE) 
  { fastcall_relation2(self,_oid_(x),y);
     } 
  else{ GC_BIND;
    { bag * l1 = GC_OBJECT(bag,OBJECT(bag,slot_get_object(x,n,Kernel._object)));
  ...
  { ClaireBoolean *Result ;
    if (Kernel._set == l->isa)
     { if (contain_ask_set(((set *) l),y) != CTRUE)
       { set * l1 = GC_OBJECT(set,GC_OBJECT(set,((set *) ((self->store_ask == CTRUE) ?
          copy_bag(l) :
          l )))->addFast(y));
        store_object(x,
          n,
...
// known ?
/* The c++ function for: known?(self:property,x:object) [0] */
...
/* The c++ function for: unknown?(self:property,x:object) [0] */
...
{ GC_BIND;
  { OID Result = 0;
    { ClaireObject * s = GC_OBJECT(ClaireObject,_at_property1(self,OWNER(_oid_(x))));
      ...
      else { bag * l1 = GC_OBJECT(bag,OBJECT(bag,slot_get_object(x,CLREAD(slot,s,index),Kernel._object)));
          bag * l = GC_OBJECT(bag,delete_bag(((self->store_ask == CTRUE) ?
            copy_bag(l1) :
            l1 ),y));
          store_object(x,
            CLREAD(slot,s,index),
...
{ GC_BIND;
  { OID Result = 0;
    { ClaireObject * s = GC_OBJECT(ClaireObject,_at_property1(self,OWNER(_oid_(x))));
      ...
      else { OID  y = GC_OID(slot_get_object(x,CLREAD(slot,s,index),OBJECT(ClaireClass,(*Kernel.srange)(_oid_(s)))));
...
/* The c++ function for: set_range(p:property,c:class,r:type) [SLOT_UPDATE] */
...
/* The c++ function for: put_store(self:property,x:object,y:any,b:boolean) [NEW_ALLOC+SLOT_UPDATE] */
void  put_store_property2(property *self,ClaireObject *x,OID y,ClaireBoolean *b)
{ GC_BIND;
  { ClaireObject * s = GC_OBJECT(ClaireObject,_at_property1(self,OWNER(_oid_(x))));
    if (Kernel._slot == s->isa)
     { OID  z = GC_OID(slot_get_object(x,CLREAD(slot,s,index),CLREAD(slot,s,srange)));
      if (equal(z,y) != CTRUE)
       store_object(x,
        CLREAD(slot,s,index),
        CLREAD(slot,s,srange),
...
/* The c++ function for: insert_definition(p:property,r:restriction) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
/* The c++ function for: initialize(x:restriction,d:class,l:list) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
/* The c++ function for: initialize(x:restriction,l:list) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
/* The c++ function for: hashinsert(m:restriction) [BAG_UPDATE+SLOT_UPDATE] */
...
/* The c++ function for: hashinsert(c:class,x:method) [BAG_UPDATE+SLOT_UPDATE] */
...
/* The c++ function for: hashinsert(l:list,x:method) [NEW_ALLOC+BAG_UPDATE] */
        ...
        { GC_LOOP;
          if (((*(l))[i] == CNULL) || 
           ...
           { if ((hashsize_list(l)*3) > (l->length*2))
             { list * myl2;
              { { list * g0020 = l;
                  list * g0021 = GC_OBJECT(list,make_list_integer((*Kernel._star)(GC_OID((*(g0020))[0]),
                    2),CNULL));
                  { OID gc_local;
                    ITERATE(g0022);
...
/* The c++ function for: hashget(c:class,p:property) [0] */
...
/* The c++ function for: @(self:property,x:class) [0] */
...
/* The c++ function for: find_which(p:property,n:integer,c:class) [0] */
