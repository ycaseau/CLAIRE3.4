/***** CLAIRE Compilation of file c:\claire\v3.4\src\meta\syntax.cl 
...
//
/* The c++ function for: combine(x:any,y:any,z:any) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
OID  combine_any(OID x,OID y,OID z)
{ GC_BIND;
  { OID Result = 0;
    { OID  p = GC_OID(operation_I_any(x));
...
{ GC_BIND;
  { OID Result = 0;
    if (y == _oid_(Reader.as))
     ...
     { if (INHERIT(OWNER(x),Core._global_variable))
       ...
       { if ((INHERIT(OWNER(z),Language._Call)) && (((OBJECT(Call,z)->selector == Kernel.add) || 
         ...
         { Call * V_CL0022;{ list * g0023UU;
            { OID v_bag;
              GC_ANY(g0023UU= list::empty(Kernel.emptySet));
              { if (OBJECT(Call,x)->args->length == 1)
                 v_bag = _oid_(OBJECT(Call,x)->selector);
                else v_bag = (*(OBJECT(Call,x)->args))[1];
                  GC_OID(v_bag);} 
              ((list *) g0023UU)->addFast(v_bag);
              { if (OBJECT(Call,x)->args->length == 1)
                 v_bag = (*(OBJECT(Call,x)->args))[1];
                else v_bag = (*(OBJECT(Call,x)->args))[2];
                  GC_OID(v_bag);} 
              ((list *) g0023UU)->addFast(v_bag);
              ((list *) g0023UU)->addFast(GC_OID((*(OBJECT(bag,(*Core.args)(z))))[2]));} 
            V_CL0022 = Call_I_property(OBJECT(property,(*Kernel.selector)(z)),g0023UU);
            } 
          
          Result=_oid_(V_CL0022);} 
        else if (OBJECT(Call,x)->selector == Kernel.nth)
         Result = _oid_(Call_I_property(Kernel.nth_equal,GC_OBJECT(list,GC_OBJECT(list,((list *) copy_bag(OBJECT(Call,x)->args)))->addFast(z))));
                    ...
                    { GC_LOOP;
                      { OID  g0028UU;
                        { Assign * _CL_obj = ((Assign *) GC_OBJECT(Assign,new_object_class(Language._Assign)));
                          ...
                          g0028UU = _oid_(_CL_obj);
                          } 
                        i_bag->addFast(g0028UU);
                        } 
                      ++i;
                      GC_UNLOOP;} 
                    } 
                  } 
                g0027 = GC_OBJECT(list,i_bag);
                } 
              (g0026->args = g0027);} 
     ...
     Result = _oid_(Call_I_property(Kernel._Z,GC_OBJECT(list,list::alloc(2,x,z))));
    else { Call * V_CL0029;{ Call_star * g0030UU;
          ...
          V_CL0029 = DBregister_Call(g0030UU);
          } 
        
        Result=_oid_(V_CL0029);} 
      GC_UNBIND; return (Result);} 
  } 
...
/* The c++ function for: operand!(x:any,n:integer) [NEW_ALLOC+SLOT_UPDATE] */
    ...
    if (INHERIT(OWNER(x),Language._Or))
     ...
     { if (OBJECT(Call,x)->selector == Core.write)
       ...
       { if (n == 2)
        ...
        else Result = _oid_(Call_I_property(Kernel.nth,GC_OBJECT(list,rmlast_list(((list *) copy_bag(OBJECT(Call,x)->args))))));
          } 
       ...
       { Variable * v = GC_OBJECT(Variable,extract_variable_any(GC_OID(OBJECT(Assign,_Zdef)->var)));
        ...
        { { Let * _CL_obj = ((Let *) GC_OBJECT(Let,new_object_class(Language._Let)));
            ...
            { Let * g0031 = _CL_obj; 
              OID  g0032;
              if (firstc_meta_reader(r) == 44)
               g0032 = readlet_meta_reader(cnext_meta_reader(r),e);
              else g0032 = nexts_meta_reader(r,e);
                (g0031->arg = g0032);} 
        ...
        { Let * g0035 = _CL_obj; 
          ...
          { Do * _CL_obj = ((Do *) GC_OBJECT(Do,new_object_class(Language._Do)));
            { Do * g0037 = _CL_obj; 
              list * g0038;
              { OID v_bag;
                GC_ANY(g0038= list::empty(Kernel._any));
                ((list *) g0038)->addFast(_Zdef);
                { { Let * _CL_obj = ((Let *) GC_OBJECT(Let,new_object_class(Language._Let)));
                    (_CL_obj->var = v2);
                    (_CL_obj->value = _Ze);
                    { Let * g0039 = _CL_obj; 
...
//
/* The c++ function for: readlet*(r:meta_reader,l:list,n:integer,e:keyword) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
        ...
        { { When * _CL_obj = ((When *) GC_OBJECT(When,new_object_class(Language._When)));
            ...
            { When * g0041 = _CL_obj; 
              ...
              if (boolean_I_any(stop_ask_integer(firstc_meta_reader(r))) == CTRUE)
               g0042 = CNULL;
              else g0042 = nexts_meta_reader(r,e);
                (g0041->other = g0042);} 
        ...
        if ((firstc_meta_reader(r) == 44) || 
         ...
         g0044 = Kernel.cfalse;
            ...
            if (x == _oid_(Reader._cl_if))
             g0044 = readif_meta_reader(r,e);
            else if (keyword_ask_any(x) == CTRUE)
             g0044 = nextstruct_meta_reader(r,OBJECT(keyword,x),Reader.none);
            else g0044 = loopexp_meta_reader(r,x,Reader.none,CFALSE);
              } 
          (g0043->other = g0044);} 
...
//
/* The c++ function for: readset(r:meta_reader,%a1:any) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
        ...
        if (equal(_Za2,r->comma) == CTRUE)
         { bag * V_CL0045;{ set * g0046UU;
            { set * u_bag = set::empty(Kernel.emptySet);
              ...
              g0046UU = GC_OBJECT(set,u_bag);
              } 
            V_CL0045 = cast_I_bag(g0046UU,Kernel.emptySet);
            } 
          
          Result=_oid_(V_CL0045);} 
         ...
         { Variable * v = GC_OBJECT(Variable,extract_variable_any(GC_OID(nexts_I_meta_reader1(r,Reader.in))));
          { OID  g0049UU;
            { Image * _CL_obj = ((Image *) GC_OBJECT(Image,new_object_class(Language._Image)));
              ...
              g0049UU = _oid_(_CL_obj);
              } 
            Result = lexical_build_any(g0049UU,GC_OBJECT(list,list::alloc(1,_oid_(v))),0);
            } 
          } 
...
{ { OID Result = 0;
    if (INHERIT(OWNER(x),Kernel._unbound_symbol))
     { OID  V_CL0050;close_exception(((general_error *) (*Core._general_error)(_string_("[170] cannot use ~S in a set constant"),
        _oid_(list::alloc(1,x)))));
      
      Result=_void_(V_CL0050);} 
    else if (INHERIT(OWNER(x),Language._Variable))
     { OID  V_CL0051;close_exception(((general_error *) (*Core._general_error)(_string_("[170] cannot use a variable (~S) in a set constant"),
        _oid_(list::alloc(1,x)))));
...
//
/* The c++ function for: readblock(r:meta_reader,x:any,e:integer) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
     ...
     { If * _Zi = GC_OBJECT(If,OBJECT(If,readif_meta_reader(r,e)));
      { OID  g0052UU;
        { if (boolean_I_any(eval_any(GC_OID(_Zi->test))) == CTRUE)
           g0052UU = _Zi->arg;
          else g0052UU = _Zi->other;
            GC_OID(g0052UU);} 
        Result = readblock_meta_reader(r,g0052UU,e);
        } 
      } 
...
/* The c++ function for: Do!(x:any,y:any) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
{ GC_BIND;
  { Call *Result ;
    { Call * g0053UU;
      ...
      Result = DBregister_Call(g0053UU);
      } 
    GC_UNBIND; return (Result);} 
  } 
...
{ GC_BIND;
  { OID Result = 0;
    { OID  n = GC_OID(skipc_meta_reader(r));
      OID  z;
      { if (old_ask == CTRUE)
...
{ GC_BIND;
  skipc_meta_reader(r);
  { Defclass *Result ;
    { OID  c = GC_OID(verify_any(_oid_(Kernel._class),GC_OID(read_ident_port(r->fromp)),_oid_(Language._Defclass)));
      Defclass * y;
