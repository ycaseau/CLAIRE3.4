/***** CLAIRE Compilation of file c:\claire\v3.4\src\meta\read.cl 
...
//
/* The c++ function for: nextunit(r:meta_reader) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
//
/* The c++ function for: loopexp(r:meta_reader,x:any,e:keyword,loop:boolean) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
/* The c++ function for: extended_operator(p:property,x:any,y:any) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
       ...
       { Quote * _CL_obj = ((Quote *) GC_OBJECT(Quote,new_object_class(Language._Quote)));
        ...
        Result = _oid_(_CL_obj);
          ...
          { if (n == 34)
            ...
            else { y= GC_OID(read_ident_port(r->fromp));
           ...
           { if (extended_comment_ask_meta_reader(r,string_v(y)) == CTRUE)
             ...
             Result = y;
                ...
                { GC_LOOP;
                  ...
                  else { OID  y = GC_OID(read_ident_port(cnext_meta_reader(r)->fromp));
...
//
/* The c++ function for: nexti(r:meta_reader,val:any) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
{ GC_BIND;
  { OID Result = 0;
    if (firstc_meta_reader(r) == 40)
     ...
     { OID  _Za1 = GC_OID(read_ident_port(cnext_meta_reader(r)->fromp));
...
// *   Part 4: miscellaneous                                            *
// **********************************************************************
/* The c++ function for: nextvariable(r:meta_reader,val:any) [NEW_ALLOC] */
    ...
    { int  i = get_string(s,_char_(']'));
      ...
      if ((i == 3) && 
       ...
       { princ_string("assert(");
        ...
        { OID  s2 = GC_OID(read_string(GC_STRING(end_of_print_void())));
