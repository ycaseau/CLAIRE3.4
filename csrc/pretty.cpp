/***** CLAIRE Compilation of file c:\claire\v3.4\src\meta\pretty.cl 
...
{ GC_BIND;
  { OID Result = 0;
    if (INHERIT(owner_any(get_symbol(self->name)),Kernel._thing))
     Result = eval_any(GC_OID(get_symbol(self->name)));
...
/* The c++ function for: write_value(self:global_variable,val:any) [NEW_ALLOC+SLOT_UPDATE] */
...
{ if ((INHERIT(OWNER(self),Kernel._thing)) || 
  ...
  { { OID Result = 0;
    ...
    if (INHERIT(OWNER(self),Language._Variable))
     ...
     { OID  s = GC_OID(lexical_change_any(_oid_(OBJECT(Call,self)->selector),lvar));
     ...
     { ClaireClass * _Ztype = OBJECT(ClaireObject,self)->isa;
      ...
      { OID gc_local;
        ...
        { GC_LOOP;
          { OID  x = GC_OID(get_slot(OBJECT(slot,s),OBJECT(ClaireObject,self)));
...
//
/* The c++ function for: iClaire/extract_symbol(self:any) [0] */
...
// is a property.
//
/* The c++ function for: iClaire/make_a_property(self:any) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
  ...
  { property *Result ;
    { ClaireObject *V_CC ;
      if (INHERIT(OWNER(self),Core._global_variable))
       ...
       { OID  x = GC_OID(get_symbol(OBJECT(symbol,self)));
...
{ GC_BIND;
  { OID Result = 0;
    { char * buffer = GC_STRING(end_of_print_void());
      princ_string(buffer);
...
/* The c++ function for: set_level(_CL_obj:void) [SLOT_UPDATE] */
void  set_level_void()
{ (Core.pretty->index = (buffer_length_void()-1));
  } 


/* The c++ function for: set_level(n:integer) [SLOT_UPDATE] */
...
//
/* The c++ function for: printbox(self:bag) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
OID  printbox_bag2(bag *self)
{ return (printbox_bag1(self,buffer_length_void(),Core.pretty->width,", "));} 


/* The c++ function for: printbox(self:bag,s:string) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
OID  printbox_bag3(bag *self,char *s)
{ return (printbox_bag1(self,buffer_length_void(),Core.pretty->width,s));} 


/* The c++ function for: printl(self:bag,s:string) [NEW_ALLOC+SLOT_UPDATE] */
...
/* The c++ function for: printexp(self:any,comp:boolean) [NEW_ALLOC+SLOT_UPDATE] */
...
{ GC_BIND;
  if (equal(_oid_(of_bag(self)),_oid_(Kernel.emptySet)) != CTRUE)
   { princ_string("list<");
    print_any(GC_OID(_oid_(of_bag(self))));
    princ_string(">");
    } 
  { princ_string("(");
    printbox_bag2(self);
