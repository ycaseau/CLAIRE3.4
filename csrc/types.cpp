/***** CLAIRE Compilation of file c:\claire\v3.4\src\meta\types.cl 
...
// *********************************************************************
// ----------------------- useful methods ------------------------------
/* The c++ function for: finite?(self:type) [0] */
...
/* The c++ function for: enumerate(self:any) [NEW_ALLOC] */
...
/* The c++ function for: sort!(x:type) [0] */
...
/* The c++ function for: finite?(self:Union) [0] */
ClaireBoolean * finite_ask_Union(Union *self)
{ return ((((OBJECT(ClaireBoolean,(*Core.finite_ask)(_oid_(self->t1)))) == CTRUE) ? (((OBJECT(ClaireBoolean,(*Core.finite_ask)(_oid_(self->t2)))) == CTRUE) ? CTRUE: CFALSE): CFALSE));} 
...
/* The c++ function for: finite?(self:Param) [0] */
...
// v3.2
/* The c++ function for: finite?(self:subtype) [0] */
ClaireBoolean * finite_ask_subtype(subtype *self)
{ return (((self->arg == Kernel._set) ? (((OBJECT(ClaireBoolean,(*Core.finite_ask)(_oid_(self->t1)))) == CTRUE) ? CTRUE: CFALSE): CFALSE));} 
...
// of types l2
/* The c++ function for: nth(self:class,l1:list,l2:list) [NEW_ALLOC+SLOT_UPDATE] */
...
// tuple are types
/* The c++ function for: finite?(self:tuple) [0] */
...
/* The c++ function for: get(self:Reference,y:any) [NEW_ALLOC+RETURN_ARG] */
...
/* The c++ function for: set!(x:class) [NEW_ALLOC] */
...
// interval
/* The c++ function for: set!(x:Interval) [0] */
                  ...
                  { OID gc_local;
                    ...
                    { GC_LOOP;
                      { OID gc_local;
                        ...
                        { GC_LOOP;
                          l2= l2->addFast(_oid_(GC_OBJECT(list,((list *) copy_bag(OBJECT(bag,l3))))->addFast(z)));
                          GC_UNLOOP;} 
                        } 
                      GC_UNLOOP;} 
                    } 
    ...
    { ClaireObject *V_CC ;
      if (INHERIT(y->isa,Core._Union))
       ...
       V_CC = CTRUE;
          ...
          PUSH(x);
          PUSH(_oid_(y));
          { ClaireObject * m = GC_OBJECT(ClaireObject,find_which_property(Kernel._Z,start,OWNER(x)));
...
/* The c++ function for: class!(x:type) [0] */
...
/* The c++ function for: U(x:type,y:type) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
/* The c++ function for: glb(x:Union,y:type) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
/* The c++ function for: glb(x:Interval,y:type) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
/* The c++ function for: glb(x:class,y:type) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
    ...
    if ((x->open == ClEnv->ABSTRACT) && 
     ...
     { list * g0115UU;
      { { bag *v_list; OID v_val;
          OID z,CLcount;
          v_list = x->subclass;
           g0115UU = v_list->clone();
          for (CLcount= 1; CLcount <= v_list->length; CLcount++)
          { z = (*(v_list))[CLcount];
            v_val = (*Core.glb)(z,
              _oid_(y));
            
            (*((list *) g0115UU))[CLcount] = v_val;} 
          } 
        GC_OBJECT(list,g0115UU);} 
      Result = Uall_list(g0115UU);
      } 
...
/* The c++ function for: glb(x:Param,y:type) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
ClaireType * glb_Param(Param *x,ClaireType *y)
{ GC_RESERVE(9);  // v3.0.55 optim !
  { ClaireType *Result ;
    if (INHERIT(y->isa,Core._Param))
     { ClaireType * c = GC_OBJECT(ClaireType,join_class(x->arg,CLREAD(Param,y,arg)));
     ...
     { ClaireType * c = GC_OBJECT(ClaireType,join_class(x->arg,((ClaireClass *) y)));
      if (equal(_oid_(c),_oid_(Kernel.emptySet)) != CTRUE)
       { Param * _CL_obj = ((Param *) GC_OBJECT(Param,new_object_class(Core._Param)));
        (_CL_obj->arg = ((ClaireClass *) c));
...
/* The c++ function for: glb(x:subtype,y:type) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
ClaireType * glb_subtype(subtype *x,ClaireType *y)
{ GC_BIND;
  { ClaireType *Result ;
    if (INHERIT(y->isa,Kernel._class))
     { if (equal(_oid_(join_class(x->arg,((ClaireClass *) y))),_oid_(Kernel.emptySet)) != CTRUE)
       Result = nth_class1(GC_OBJECT(ClaireClass,((ClaireClass *) join_class(x->arg,((ClaireClass *) y)))),GC_OBJECT(ClaireType,x->t1));
      else Result = Kernel.emptySet;
        } 
    else if (INHERIT(y->isa,Core._Param))
     { if (equal(_oid_(join_class(x->arg,CLREAD(Param,y,arg))),_oid_(Kernel.emptySet)) != CTRUE)
       Result = param_I_class(GC_OBJECT(ClaireClass,((ClaireClass *) join_class(x->arg,CLREAD(Param,y,arg)))),GC_OBJECT(ClaireType,OBJECT(ClaireType,(*Core.glb)(GC_OID(_oid_(member_type(x))),
     ...
     { if (equal(_oid_(join_class(x->arg,CLREAD(subtype,y,arg))),_oid_(Kernel.emptySet)) != CTRUE)
       { OID  t = GC_OID((*Core.glb)(GC_OID(_oid_(x->t1)),
        ...
        if (equal(t,_oid_(Kernel.emptySet)) != CTRUE)
         Result = nth_class1(GC_OBJECT(ClaireClass,((ClaireClass *) join_class(x->arg,CLREAD(subtype,y,arg)))),OBJECT(ClaireType,t));
        else Result = Kernel.emptySet;
          } 
      else Result = Kernel.emptySet;
        } 
...
// set, Interval, list
/* The c++ function for: glb(x:tuple,y:type) [NEW_ALLOC] */
     ...
     { if (CLREAD(subtype,y,arg) == Kernel._tuple)
       { list * g0116UU;
        { { bag *v_list; OID v_val;
            OID z,CLcount;
            v_list = x;
             g0116UU = v_list->clone();
            for (CLcount= 1; CLcount <= v_list->length; CLcount++)
            { z = (*(v_list))[CLcount];
              v_val = (*Core.glb)(z,
                GC_OID(_oid_(CLREAD(subtype,y,t1))));
              
              (*((list *) g0116UU))[CLcount] = v_val;} 
            } 
          GC_OBJECT(list,g0116UU);} 
        Result = tuple_I_list(g0116UU);
        } 
      else Result = Kernel.emptySet;
        } 
    else Result = OBJECT(ClaireType,(*Core.glb)(_oid_(y),
    ...
    { OID  itest;
      ...
      if (itest != CNULL)
       { int  i = itest;
        ...
        tuple * x1 = GC_OBJECT(tuple,((tuple *) copy_bag(x)));
        tuple * x2 = GC_OBJECT(tuple,((tuple *) copy_bag(x)));
       ...
       { ClaireBoolean *v_and;
        { v_and = ((x->length == ((bag *) y)->length) ? CTRUE : CFALSE);
          if (v_and == CFALSE) Result =CFALSE; 
              ...
              { int  i = 1;
                int  g0121 = x->length;
                { g0123UU= _oid_(CFALSE);
                  while ((i <= g0121))
                  { if ((*Core._inf_equalt)((*(x))[i],
                      (*(((bag *) y)))[i]) != Kernel.ctrue)
                     { g0123UU = Kernel.ctrue;
                      break;} 
                    ++i;
                    } 
                  } 
                } 
              v_and = not_any(g0123UU);
              } 
            if (v_and == CFALSE) Result =CFALSE; 
            else Result = CTRUE;} 
          } 
     ...
     { ClaireBoolean *v_and;
      { v_and = OBJECT(ClaireBoolean,_oid_((ClaireObject *) Core._inf_equalt->fcall(((int) x),((int) CLREAD(Param,y,arg)))));
        if (v_and == CFALSE) Result =CFALSE; 
            ...
            { int  n = 1;
              int  g0124 = CLREAD(Param,y,params)->length;
              { OID gc_local;
                g0125UU= _oid_(CFALSE);
                while ((n <= g0124))
                { GC_LOOP;
                  if ((*Core._inf_equalt)(GC_OID(_oid_(_at_type(x,OBJECT(property,(*(CLREAD(Param,y,params)))[n])))),
                   ...
                   { g0125UU = Kernel.ctrue;
                    break;} 
                  ++n;
                  GC_UNLOOP;} 
                } 
              } 
            v_and = not_any(g0125UU);
            } 
          if (v_and == CFALSE) Result =CFALSE; 
          else Result = CTRUE;} 
        } 
...
/* The c++ function for: member(x:type) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
     ...
     { list * g0126UU;
      { { bag *v_list; OID v_val;
          ...
          { y = (*(v_list))[CLcount];
            ...
            (*((list *) g0126UU))[CLcount] = v_val;} 
          } 
        GC_OBJECT(list,g0126UU);} 
      Result = Uall_list(g0126UU);
      } 
    else Result = Kernel.emptySet;
      GC_UNBIND; return (Result);} 
  } 
...
/* The c++ function for: of_extract(x:type) [0] */
...
/* The c++ function for: @(x:type,p:property) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
    ...
    if (INHERIT(x->isa,Kernel._class))
     ...
     { list * g0127UU;
      { { bag *v_list; OID v_val;
          OID y,CLcount;
          v_list = ((bag *) x);
           g0127UU = v_list->clone();
          for (CLcount= 1; CLcount <= v_list->length; CLcount++)
          { y = (*(v_list))[CLcount];
            v_val = _oid_(set::alloc(1,GC_OID(funcall_property(p,y))));
            
            (*((list *) g0127UU))[CLcount] = v_val;} 
          } 
        GC_OBJECT(list,g0127UU);} 
      Result = Uall_list(g0127UU);
      } 
...
/* The c++ function for: the(x:type) [NEW_ALLOC] */
...
/* The c++ function for: abstract_type(p:operation,xt1:type,xt2:type) [BAG_UPDATE+SLOT_UPDATE] */
  ...
  { ClaireType *Result ;
    if (Kernel._set == xt1->isa)
     { Result = ((equal(_oid_(xt1),_oid_(Kernel.emptySet)) != CTRUE) ?
        abstract_type_operation(p,abstract_type_set(((set *) xt1)),xt2) :
        xt1 );
      } 
     ...
     { if (INHERIT(xt2->isa,Core._Interval))
       ...
       { Result = ((equal(_oid_(xt2),_oid_(Kernel.emptySet)) != CTRUE) ?
          abstract_type_operation(p,xt1,abstract_type_set(((set *) xt2))) :
          xt2 );
        } 
      else if (INHERIT(xt2->isa,Core._Union))
       Result = U_type(GC_OBJECT(ClaireType,abstract_type_operation(p,xt1,CLREAD(Union,xt2,t1))),GC_OBJECT(ClaireType,abstract_type_operation(p,xt1,CLREAD(Union,xt2,t2))));
      else Result = Kernel._integer;
        } 
    else if (INHERIT(xt1->isa,Core._Union))
     Result = U_type(GC_OBJECT(ClaireType,abstract_type_operation(p,CLREAD(Union,xt1,t1),xt2)),GC_OBJECT(ClaireType,abstract_type_operation(p,CLREAD(Union,xt1,t2),xt2)));
    else Result = Kernel._integer;
      GC_UNBIND; return (Result);} 
  } 
...
/* The c++ function for: meet_arg_types(x:type,y:type) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
ClaireType * meet_arg_types_type(ClaireType *x,ClaireType *y)
{ return (U_type(x,y));} 


/* The c++ function for: first_member_type(x:type,y:type) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
/* The c++ function for: nth(self:array,x:integer) [0] */
OID  nth_array(OID *self,int x)
{ { OID Result = 0;
    if ((x > 0) && 
        (x <= self[0]))
     Result = nth_get_array(self,x);
    else { OID  V_CL0129;close_exception(((general_error *) (*Core._general_error)(_string_("[180] nth[~S] out of scope for ~S"),
          _oid_(list::alloc(2,x,_array_(self))))));
        
        Result=_void_(V_CL0129);} 
      return (Result);} 
  } 
...
/* The c++ function for: make_set(self:array[of:(any)]) [NEW_ALLOC] */
