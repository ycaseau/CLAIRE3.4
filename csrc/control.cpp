/***** CLAIRE Compilation of file c:\claire\v3.4\src\meta\control.cl 
...
/* The c++ function for: printelse(self:If) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
//--------------------- block structure------------------------------
/* The c++ function for: self_print(self:Do) [NEW_ALLOC+SLOT_UPDATE] */
...
/* The c++ function for: self_print(self:Let) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
/* The c++ function for: self_eval(self:Let) [NEW_ALLOC] */
...
//
/* The c++ function for: self_print(self:When) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
/* The c++ function for: self_eval(self:When) [NEW_ALLOC] */
...
/* The c++ function for: self_eval(self:For) [NEW_ALLOC] */
        ...
        { if (INHERIT(OWNER(x),Kernel._class))
           ...
           { int  n = array_v(x)[0];
            ...
            { OID gc_local;
              ...
              while ((g0044 <= g0045))
              { GC_LOOP;
                { OID  z = GC_OID(nth_get_array(array_v(x),g0044));
                  write_value_Variable(self->var,z);
                  eval_any(GC_OID(self->arg));
                  } 
                ++g0044;
...
// *********************************************************************
// ----------------- case  --------------------------------------
/* The c++ function for: self_print(self:Case) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
/* The c++ function for: self_print(self:While) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
//
/* The c++ function for: self_print(self:Handle) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE] */
...
/* The c++ function for: self_print(self:Construct) [NEW_ALLOC+SLOT_UPDATE] */
          ...
          { OID gc_local;
            ...
            { GC_LOOP;
              { OID  m = GC_OID((*Kernel.nth)(s,
                ...
                else if (_oid_(_char_('F')) == m)
                 { OID  fv = GC_OID(OPT_EVAL((*(l))[i]));
                  ClaireBoolean * p_Z = CFALSE;
                  int  j = (integer_I_char(_char_(string_v(s)[(*Core._plus)(n,
                    2) - 1]))-48);
                  if (_oid_(_char_('%')) == (*Kernel.nth)(s,
                    GC_OID((*Core._plus)(n,
                      2))))
                   { p_Z= CTRUE;
                    j= 1;
                    GC__OID(fv = (*Kernel._star)(fv,
                      GC_OID(_float_(100.0))), 7);
                    } 
                  else if ((j < 0) || 
                      (j > 9))
                   close_exception(((general_error *) (*Core._general_error)(_string_("[189] F requires a single digit integer in ~S"),
                    _oid_(list::alloc(1,_oid_(self))))));
                  if ((p_Z != CTRUE) && 
                      (_oid_(_char_('%')) == (*Kernel.nth)(s,
                        GC_OID((*Core._plus)(n,
                          3)))))
                   { p_Z= CTRUE;
                    GC__OID(fv = (*Kernel._star)(fv,
                      GC_OID(_float_(100.0))), 7);
                    GC__OID(n = (*Core._plus)(n,
                      1), 5);
                    } 
                  (*Core.printFDigit)(fv,
                    j);
...
//
/* The c++ function for: self_eval(self:Assert) [NEW_ALLOC] */
